<?php if ($modal=="ubah_password_admin"): ?>
  ...
  $("#formValidate").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
          type: "POST",data:values,url: "<?php echo base_url('Superadmin/ubah_password_admin'); ?>",
...
<?php endif; ?>
<?php if ($modal=="ubah_password_user"): ?>
    <form class="formValidate_password" id="formValidate_password">
        <div class="progress" id="load-progress-pesan" style="margin: 0 0 0 0;display:none">
            <div class="indeterminate" id="progressBar"></div>
        </div>
        <div class="modal-content">
    ...
    <script type="text/javascript">
        $("#formValidate_password").validate({
            rules: {
                password_baru:  {
                    required: true,
                    minlength: 6
            ...
            submitHandler: function(form) {
                var values = $('#formValidate_password').serialize();
                $('#load-progress-pesan').show();
                $.ajax({
                    type: "POST",data:values,url: "<?php echo base_url('Superadmin/ubah_password_user'); ?>",
                    success: function(data)
                    {
                        $('#load-progress-pesan').hide();
                        if (data==1) {
...
<?php endif; ?>
<?php if ($modal=="ubah_password_superadmin"): ?>
    <form class="formValidate_password" id="formValidate_password">
        ...
        <div class="modal-content">
            ...
            <div class="row">
                <div class="input-field col s12">
                    <i class="material-icons prefix">lock</i>
                    <input type="password" name="password_lama" id="password_lama">
                    <input type="hidden" name="id_user" id="id_user" value="<?php echo $id_superadmin;?>">
                    <label>Password Lama</label>
                </div>
            </div>
            <div class="row">
    ...
    <script type="text/javascript">
        $("#formValidate_password").validate({
            ...
            submitHandler: function(form) {
                ...
                $('#load-progress-pesan').show();
                $.ajax({
                    type: "POST",data:values,url: "<?php echo base_url('Superadmin/ubah_password_superadmin'); ?>",
                    success: function(data)
                    {
                        $('#load-progress-pesan').hide();
                        if (data==1) {
  ...
  $("#formValidate").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
          ...
          type: "POST",
          data:values,
          url: "<?php echo base_url('Superadmin/tambah_lokasi_tempat_umum'); ?>",
          processData : false,
          contentType : false,
          success: function(data)
          {
  ...
  $("#formValidate").validate({
    ...
    submitHandler: function(form) {
      ...
      $.ajax({
          ...
          type: "POST",
          data:values,
          url: "<?php echo base_url('Superadmin/edit_lokasi_tempat_umum'); ?>",
          success: function(data) {
            $('#load-progress-pesan').hide();
            if (data==1) {
              $('#modal_dialog').modal('close');
    ...
    submitHandler: function(form) {
      if (array_imgName.length == 0) {
        ...
        $.ajax({
            ...
            type: "POST",
            data:values,
            url: "<?php echo base_url('Superadmin/tambah_foto_lokasi_tempat_umum'); ?>",
            processData : false,
            contentType : false,
            success: function(data)
            {
...
<?php endif; ?>
<?php if ($modal=="tambah_jenis_lokasi"): ?>
  <form class="formValidate" id="formValidate">
    <div class="progress" id="load-progress-pesan" style="margin: 0 0 0 0;display:none">
      <div class="indeterminate" id="progressBar"></div>
    </div>
    <div class="modal-content">
      <center><h5><b>Tambah Jenis Lokasi</b></h5></center>
      <div class="row">
        <div class="input-field col s12">
          <i class="material-icons prefix">lock</i>
          <input type="text" name="nama" id="nama">
          <label>Nama Jenis Lokasi Tempat Umum</label>
        </div>
      </div>
      <div class="row">
        <div class="input-field col s12">
          <i class="material-icons prefix">lock</i>
          <input type="text" name="icons" id="icons">
          <label>Icon Materialize</label>
        </div>
      </div>
    </div>
    <div class="modal-footer amber lighten-4">
  ...
  $("#formValidate").validate({
    rules: {
      nama: "required", icons: "required" },
    messages: {
      nama: "Silahkan diisi...", icons: "Silahkan diisi..." },
    errorElement : 'div',
    errorPlacement: function(error, element) {
        var placement = $(element).data('error');
        if (placement) {
    ...
    submitHandler: function(form) {
      ...
      $('#load-progress-pesan').show();
      $.ajax({
          type: "POST",data:values,url: "<?php echo base_url('Superadmin/tambah_jenis_lokasi'); ?>",
          success: function(data)
          {
            $('#load-progress-pesan').hide();
            if (data==1) {

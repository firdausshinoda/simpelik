            ...
            <section id="content">
                <div class="col s12">
                    <div class="row intro" style="margin-bottom:0px;">
                        ...
                        <div class="fixed-action-btn vertical click-to-toggle" style="bottom:30px;right:53px;" id="div_add_location">
                            <a class="btn-floating btn-large pink lighten-1 waves-effect waves-light" id="add_location" onclick="enableClick()">
                        ...
                        <div class="tap-target blue lighten-1" data-activates="add_location">
                                    ...
                                    <div class="nav-wrapper" style="padding:0 5%">
                                        ...
                                        <div class="col s4 m4 l4 hide-on-med-and-down">
                                            <ul class="right">
                                                ...
                                                <li>
                                                    <a href="<?php echo base_url('notifikasi')?>" class="waves-effect waves-block waves-light notification-button tooltipped" data-activates="notifications-dropdown" data-position="bottom" data-delay="50" data-tooltip="Pemberitahuan">
                                ...
                                <div class="col s12 m12 l12">
                                    <div class="col s3 m3 l3 right">
                                        <div class="input-field right">
                                            <a class="btn-floating btn-large waves-effect waves-light grey darken-1 tooltipped" onclick="showAssigment()" data-position="bottom" data-delay="50" data-tooltip="Konfirmasi Lokasi Terbaru"><i class="material-icons">assignment</i></a>
                                        ...
                                        <ul class="collapsible collapsible-accordion display_none" id="containerMenu" data-collapsible="accordion" style="position: absolute;width:20%;right:1%;">
                                            ...
                                            <script type="text/javascript">
                                                ...
                                                $.each(<?php echo $dt_lokasi; ?>, function( key, value ) {
                                                    dt_lokasi.push([value.nama,value.lt,value.lg,value.id_jenis_lokasi,value.img_lokasi,value.id_lokasi]);
                                                });
                                            </script>
                                            <?php $i=0; foreach ($dt_jenis_lokasi as $val): $i++;?>
                                                <li>
                                                    <div class="collapsible-header">
                                                        ...
                                                        <span class="white-text  badge purple darken-1"><?php echo $dt_jenis_lokasi[$i]['jml']; ?></span>
                                                    </div>
                                    ...
                                    <div class="col s12 m12 l12 display_none" id="container_check">
                                        <div style="position: absolute;width:45%;right:1%;" class="grey lighten-2">
                                            <div class="progress grey darken-1 display_none" id="load-progress-assigment" style="margin: 1.5% 0 0 0;position: absolute;top: 9.8%;z-index:1">
                                                <div class="indeterminate" id="progressBar"></div>
                                            </div>
                                            <ul class="collapsible collapsible-accordion" style="position: absolute;width:100%;">
                                                <li>
                                                    <div class="collapsible-header grey darken-1 white-text">
                                                        <i class="material-icons">map</i>Konfirmasi Lokasi Tempat Umum
                                                    </div>
                                                </li>
                                            </ul>
                                            <div id="viewAssigment"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    var stt_boolan_show_assigment=false;
    var map;
    var all_marker=[];
    ...
    $("#search").keyup(function () {
        ...
        $.ajax({
            type: "POST",
            data: {cari: cari},
            url: "<?php echo base_url('Admin/cari_lokasi_tempat_umum'); ?>",
            dataType: "json",
                        ...
                        for (var i = 0; i < nmLokasi.length; i++) {
                            if (nmLokasi[i] == txt) {
                                ...
                                var contentString = '<div id="content">'+
                                    ...
                                    '<button class="btn lime darken-4 waves-effect waves-light right col s12 m12 l12" onclick="openSideNav('+idLokasi[i]+')" type="button">Detail</button>'
                                '</div>'
                                '</div>';
...
<script type="text/javascript">
    ...
    function showMenu() {
        if (stt_boolan_show_menu==false) {
            ...
            if (stt_boolan_show_assigment==true){
                $('#container_check').hide(1000);
            }
        } else {
            $('#containerMenu').hide(1000);
            stt_boolan_show_menu=false;
            if (stt_boolan_show_assigment==true){
                $('#container_check').show(1000);
            }
        }
    }

    function showAssigment() {
        if (stt_boolan_show_assigment==false){
            $('#container_check').show(1000);
            stt_boolan_show_assigment=true;
            if (stt_boolan_show_menu==true){
                $('#containerMenu').hide(1000);
            }
            setAssigment();
        } else {
            $('#container_check').hide(1000);
            stt_boolan_show_assigment=false;
            if (stt_boolan_show_menu==true){
                $('#containerMenu').show(1000);
            }
        }
    ...
    function setMapMarker(str,_marker,id) {
        for (var i = 0; i < _marker.length; i++) {
            ...
            if (str=='true') {
                if (_m_ki[3]==id){
                    addMarker(_m_ki[0],lat_lng,_m_ki[3],_m_ki[4],_m_ki[5]);
                }
            } else {
                delMarker(id);
            }
...
//                autocomplete.setBounds(new google.maps.LatLngBounds(geolocation, geolocation));
    ...
    function addMarker(title,location,idJns_Lokasi,img,idLokasi) {
        var marker = new google.maps.Marker({
            position: location,
            map: map,
            title:title,
        ...
        var contentString = '<div id="content">'+
            ...
            '<button class="btn grey darken-3 waves-effect waves-light right col s12 m12 l12" onclick="openSideNav(\''+idLokasi+'\')" type="button">Detail</button></div></div>';
        var infowindow = new google.maps.InfoWindow({
            content: contentString
        });
        marker.addListener('click', function() {
    ...
    function redirectOpen(str) {
        window.open("<?php echo base_url('Aduan')."/"?>"+str);
    }
    ...
    function openSideNav(str) {
        var geocoder = new google.maps.Geocoder;
        $('#load-progress-sidenav').show();
        $('.button-collapse').sideNav('show');
        $.ajax({
            type: "GET",data:{id:str},url: "<?php echo base_url('Admin/detail_lokasi_tempat_umum'); ?>",
            dataType: "JSON",
            success: function(data){
    ...
    function enableClick(){
        ...
        $("#div_add_location").hide('5000');
        $("#div_close_location").show('1000');
    ...
    function disableClick(){
        ...
        $("#div_add_location").show('5000');
        $("#div_close_location").hide('1000');
    ...
    function setAssigment() {
        $('#load-progress-assigment').show();
        $.ajax(
            {
                type: "GET",
                url: "<?php echo base_url('Admin/setAssigment'); ?>",
                success: function(data) {
                    $('#load-progress-assigment').hide();
                    dt_assigment=[];
                    $.each(data.result, function( key, value ) {
                        dt_assigment.push([value.id_lokasi,value.img_user,value.nama_user,value.nama_lokasi,value.lati,value.longi,value.img_gallery]);
                    });
                    setViewAssigment();
                },
                error:function(data){
    ...
    function setViewAssigment() {
        var html='<ul class="collection" style="position: absolute;width:100%;height:25rem;overflow-y: auto;overflow-x:hidden;top:55px;text-align: left">';
        if (dt_assigment.length==0){
            html=html+'<li class="collection-item avatar">'+
                '<center><span>Tidak Ada Terbaru</span></center>'+
                '</li>';
        } else {
            for (i=0;i<dt_assigment.length;i++){
                html=html+'<li class="collection-item avatar">'+
                    '<img src="'+dt_assigment[i][1]+'" alt="" class="circle">'+
                    '<span class="title"><b>'+dt_assigment[i][2]+'</b></span>'+
                    '<p>'+dt_assigment[i][3]+'</p><div class="secondary-content ">'+
                    '<button type="button" class="btn red waves-effect waves-light right" onclick="konfirmAssigment('+dt_assigment[i][0]+',\''+dt_assigment[i][3]+'\',\'delete\');"><i class="material-icons">delete</i></button>'+
                    '<button type="button" class="btn blue waves-effect waves-light right" onclick="konfirmAssigment('+dt_assigment[i][0]+',\''+dt_assigment[i][3]+'\',\'konfirm\');"><i class="material-icons">done</i></button>'+
                    '<button type="button" class="btn blue-grey lighten-2 waves-effect waves-light right" onclick="setAssigment_marker('+i+')" style="margin-right: 2px;"><i class="material-icons">place</i></button>'+
                    '</div>'+
                    '</li>';
            }
        }
        html=html+'</ul>';
        $('#viewAssigment').html(html);
    }

    function setAssigment_marker(index) {
        clearMarker();
        var location = new google.maps.LatLng(dt_assigment[index][4],dt_assigment[index][5]);
        var marker = new google.maps.Marker({
            position: location,
            map: map,
            title:dt_assigment[index][3]
        });
        all_marker.push(marker);
        var contentString = '<div id="content">'+
            '<div class="row"><div class="col s12"><p id="firstHeading"><h5><b>'+dt_assigment[index][3]+'</b></h5></p>'+
            '<img src="'+dt_assigment[index][6]+'" class="responsive" style="width:150px;">'+
            '</br><div class="col s12"><center><button class="btn lime darken-4 waves-effect waves-light right" onclick="openSideNav('+dt_assigment[index][0]+')" type="button" style="margin-left:1px;">Detail</button></center></div>'+
            '</div></div></div>';
        var infowindow = new google.maps.InfoWindow({
            content: contentString
        });
    ...
    function konfirmAssigment(id,nama,jns) {
        $('#load-progress-assigment').show();
        $.ajax(
            {
                type: "POST",
                data:{id:id,nama:nama,jns:jns},
                url: "<?php echo base_url('Admin/konfirmAssigmentMaps'); ?>",
                success: function(data)
                {
                    $('#load-progress-assigment').hide();
                    setTimeout(function() {
                        Materialize.toast('<span>Konfirmasi Sukses.</span>', 3000);
                    }, 100);
                    for (i=0;i<dt_assigment.length;i++){
                        if (dt_assigment[i][0]==id){
                            dt_assigment.splice(i, 1);
                        }
                    }
                    setViewAssigment();
                },
